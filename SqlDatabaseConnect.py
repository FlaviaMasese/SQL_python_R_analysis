...
class PsqlEngine:
    def __init__(self, username: str, password: Any,
        ...
        self.connection_path = ("postgresql+psycopg2://{self.username}:{self.password}"
...
#%% fill in parameters to create database engine
        ...
        def get_table_as_dataframe(database_table_name: str) -> pd.DataFrame:
            return pd.read_sql_table(table_name=database_table_name, con=self.engine)
        
        def fetch_sqlquery_as_dataframe(sqlquery_statement: Any) -> pd.DataFrame:
            return pd.read_sql_query(sql=sqlquery_statement, con=self.engine)
        
        
        def send_data_to_sqldatabase(data, table_name: str, index=False, if_exists='replace'):
            #%% write to database
        ...
        def copy_data_to_psqldatabase(data, table_name):
